...
void eval(char *cmdline)
{
  ...
  while (i < numcmds) {
      ...
      //parent process
          ...
          if (i > 0) { close(temp); }
          if (i < numcmds - 1) { close(pipeline[1]); }
          temp = pipeline[0];

          if (pipe(pipeline) < 0 && i < numcmds - 1 && multcmds) { exit(0); }
      }

      //child process
      else if (pid == 0) {
          if (stdin_redir[i] != -1) {
              dup2(fileno(fopen(argv[stdin_redir[i]], "r")), STDIN_FILENO);
          }
          if (stdout_redir[i] != -1) {
              dup2(fileno(fopen(argv[stdout_redir[i]], "w")), STDOUT_FILENO);
          }
